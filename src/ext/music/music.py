...
from .queue import QueueLoop

import wavelink
import asyncio

class MusicExtension(Cog):
    ...
    group = discord.discord.app_commands.Group(name="music", description="A simple yet powerful music player that can play from YouTube and Spotify")

    @Cog.listener()
    async def on_wavelink_node_ready(self, node: wavelink.Node):
        log.info(f'[NODE] {node.identifier} connected.')  
    ...
    @discord.app_commands.describe(query="YouTube url or keyword to search on YouTube")
    ...
    @discord.app_commands.describe(loop="Off: Disable loop queue | All: Loop the entire queue | One: Loop only the current song")
    @discord.app_commands.choices(loop=[
        discord.app_commands.Choice(name="Off", value=0),
        discord.app_commands.Choice(name="All", value=1),
        discord.app_commands.Choice(name="One", value=2)
    ])
    async def loop_command(self, interaction: discord.Interaction, loop: discord.app_commands.Choice[int]):
        if not await self.check_interaction(interaction):
            return
        
        player = await self.manager.get_player(interaction.guild)
        if loop.value == 0:
            player.queue.loop = PlayerLoopState.NONE
            await interaction.response.send_message("Queue loop set to off")
        if loop.value == 1:
            player.queue.loop = PlayerLoopState.ALL
            await interaction.response.send_message("Queue loop set to loop all")
        if loop.value == 2:
            player.queue.loop = PlayerLoopState.ONE
